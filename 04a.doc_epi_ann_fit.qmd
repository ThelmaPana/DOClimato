---
title: "Predict annual epipelagic DOC"
...
output_filename <- "data/04.doc_epi_ann_pred.Rdata"

df_fit <- df_ann_epi_fit
df_pred <- df_ann_epi_pred
```
...
```{r roles}
# Response variable
resp_var <- c("log_doc_epi", "doc_epi") # keep both untransformed and transformed predictor
# Explanatory variables
exp_vars <- df_fit %>% select(temperature_surf:nitrate_epi) %>% colnames()
# Metadata
...
We want the response variable to be \~normally distributed. Let’s plot the distribution of log_doc_epi.

```{r resp_dist}
ggplot(df_fit) + geom_histogram(aes(x = log_doc_epi), bins = 100)
```
...
ggplot(df_fit) + 
  ...
  geom_point(aes(x = lon, y = lat, colour = log_doc_epi), size = 0.5) +
...
df_pca <- df_fit %>% select(c(lon, lat, doc_epi, log_doc_epi, everything()))
...
For epi, meso and bathypelagic, only stratified on deciles of the response variable (`doc_log`) is used.
...
folds <- nested_cv(
    df_fit,
    outside = vfold_cv(v = 10, strata = log_doc_epi, breaks = 9),
    inside = vfold_cv(v = 10, strata = log_doc_epi, breaks = 9)) %>%
...
xgb_form <- as.formula(paste("log_doc_epi ~ ", paste(c("doc_epi", exp_vars), collapse = " + "), sep = ""))
```
...
res <- lapply(1:nrow(folds), function(i){
  ...
  xgb_rec <- recipe(xgb_form, data = df_train) %>%
    update_role(doc_epi, new_role = "untransformed outcome")
  ...
  preds <- predict(final_res, new_data = df_test) %>%
    bind_cols(df_test %>% select(log_doc_epi))
  ...
  ## Predict new data
  new_preds <- augment(final_res, new_data = df_pred %>% mutate(doc_epi = NA)) %>% 
    select(-doc_epi) %>% 
