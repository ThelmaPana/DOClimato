...
# Get common colourbar limits for meso and bathy
doc_avg_lims <- c(
  min(df_2ab$doc_avg, df_2cd$doc_avg, df_2ef$doc_avg), 
  max(df_2ab$doc_avg, df_2cd$doc_avg, df_2ef$doc_avg)  
)
doc_sd_lims <- c(
  min(df_2ab$doc_sd, df_2cd$doc_sd, df_2ef$doc_sd), 
  max(df_2ab$doc_sd, df_2cd$doc_sd, df_2ef$doc_sd)  
)
...
## Assemble
p2 <- p2a + p2b + p2c + p2d + p2e + p2f + plot_annotation(tag_levels = "a") + plot_layout(guides = "collect", ncol = 2) & theme(legend.position = 'bottom', legend.spacing.x = unit(2.7, "cm"))
p2

## Save
ggsave(file = "figures/figure_2.png", plot = p2, width = 180, height = 180, unit = "mm", bg = "white")
...
# Assemble
p3 <- p3a / p3b / p3c / p3d + plot_layout(axis_titles = "collect")
p3
