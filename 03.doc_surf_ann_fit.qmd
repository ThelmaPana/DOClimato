---
title: "Predict annual surface DOC."
...
```{r filter}
## Keep only annual data, in the surface layer
df_all <- df_all %>% filter(layer == "surf" & season == 0)
df_env <- df_env %>% filter(layer == "surf" & season == 0)
```
...
exp_vars <- df_all %>% select(bbp:par) %>% colnames()
# Metadata
meta_vars <- c("lon", "lat")
```
...
ggplot(df) + 
  ...
  geom_point(aes(x = lon, y = lat, colour = doc), size = 0.5) +
