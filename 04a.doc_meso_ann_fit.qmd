---
title: "Predict annual mesopelagic DOC"
...
output_filename <- "data/04.doc_meso_ann_pred.Rdata"

df_fit <- df_ann_meso_fit
df_pred <- df_ann_meso_pred
```
...
```{r roles}
# Response variable
resp_var <- c("log_doc_meso", "doc_meso") # keep both untransformed and transformed predictor
# Explanatory variables
exp_vars <- df_fit %>% select(temperature_surf:nitrate_meso) %>% colnames()
# Metadata
...
We want the response variable to be \~normally distributed. Let’s plot the distribution of log_doc_meso.

```{r resp_dist}
ggplot(df_fit) + geom_histogram(aes(x = log_doc_meso), bins = 100)
```
...
ggplot(df_fit) + 
  ...
  geom_point(aes(x = lon, y = lat, colour = log_doc_meso), size = 0.5) +
...
df_pca <- df_fit %>% select(c(lon, lat, doc_meso, log_doc_meso, everything()))
...
folds <- nested_cv(
    df_fit,
    outside = vfold_cv(v = 10, strata = log_doc_meso, breaks = 9),
    inside = vfold_cv(v = 10, strata = log_doc_meso, breaks = 9)) %>%
...
xgb_form <- as.formula(paste("log_doc_meso ~ ", paste(c("doc_meso", exp_vars), collapse = " + "), sep = ""))
```
...
res <- pbmclapply(1:nrow(folds), function(i){
  ...
  xgb_rec <- recipe(xgb_form, data = df_train) %>%
    update_role(doc_meso, new_role = "untransformed outcome")
  ...
  preds <- predict(final_res, new_data = df_test) %>%
    bind_cols(df_test %>% select(log_doc_meso))
  ...
  ## Predict new data
  new_preds <- augment(final_res, new_data = df_pred %>% mutate(doc_meso = NA)) %>% 
    select(-doc_meso) %>% 
