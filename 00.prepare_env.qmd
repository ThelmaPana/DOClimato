  ...
  for (i in seq(along = lon)) {
    for (j in seq(along = lat)) {
      for (m in 1:4) {
        ...
        depth_idx <- depth > meso_bottom # depth indices between 200 and 1000 m
        keep <- X[i, j, depth_idx, m]
        if (sum(!is.na(keep)) > 2) {
...
# Remove nutrients in the bathypelagic as these are only NAs and convert to tibble
df_env_m <- df_env_m %>% select(-c(nitrate.bathy, silicate.bathy, phosphate.bathy)) %>% as_tibble()
...
#| out-width: 100%
ggmap(df_env_m, "temperature", type = "raster") + facet_grid(season~layer)
```
...
df_env_m[inland, !names(df_env_m) %in% c("lon", "lat", "season", "layer")] <- NA
...
#| fig-column: body-outset
#| out-width: 100%
ggmap(df_env_m, "salinity", type = "raster", land = FALSE) + facet_grid(season~layer)
ggmap(df_env_m, "density", type = "raster", land = FALSE) + facet_grid(season~layer)
ggmap(df_env_m, "npp", type = "raster", land = FALSE) + facet_grid(season~layer)
ggmap(df_env_m, "mld", type = "raster", land = FALSE) + facet_grid(season~layer)
```
...
df_env_ann <- df_env_m %>% 
  ...
  group_by(lon, lat, season, layer) %>% 
  summarise_all(mean, na.rm = TRUE) %>% 
  ungroup()

# Drop seasonal data for meso and bathy
df_env_m <- df_env_m %>% filter(layer == "surf")
...
var_names <- df_env %>% select(-c(lon, lat, season, layer)) %>% colnames()
plot_list <- list()
