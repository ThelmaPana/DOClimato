...
```{r meso_ann}
## New predictions data
# Env data in the mesoace layer, annual, to be used for new predictions. NB: this requires droping any row with missing values
df_ann_meso_pred <- df_env %>% filter(season == 0) %>% select(-contains(c("surf", "bathy"))) %>% drop_na()

## Model fitting data
# Join env data with doc data, drop missing observations, apply log transformation to doc value
df_ann_meso_fit <- df_doc %>% 
  filter(season == 0) %>% 
  select(-contains(c("surf", "bathy"))) %>% 
...
## New predictions data
# Env data in the mesoace layer, annual, to be used for new predictions. NB: this requires droping any row with missing values
df_ann_bathy_pred <- df_env %>% filter(season == 0) %>% select(-contains(c("surf", "meso"))) %>% drop_na()

## Model fitting data
# Join env data with doc data, drop missing observations, apply log transformation to doc value
df_ann_bathy_fit <- df_doc %>% 
  filter(season == 0) %>% 
  select(-contains(c("surf", "meso"))) %>% 
